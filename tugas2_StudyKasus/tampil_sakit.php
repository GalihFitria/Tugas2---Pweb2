...
<body>
    ...
    <div class="container mt-3">
        <h2>
            <center>DATA IZIN SAKIT PEGAWAI</center>
        </h2><br>
        <table class="table table-bordered">
            ...
            <tbody>
                <?php
                // Menyertakan file sakit.php yang memuat class IzinSakit
                include('sakit.php');

                // Membuat objek IzinSakit untuk mengambil data
                $IzinSakit = new IzinSakit();
                $data = $IzinSakit->tampilData(); // Mengambil data izin sakit pegawai

                // Mengecek apakah data tidak kosong
                if (!empty($data)) {
                    // Looping untuk menampilkan data izin sakit pegawai dalam tabel
                    foreach ($data as $row) {
                ?>
                        <tr>
